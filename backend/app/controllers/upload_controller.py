...
async def _read_excel_file(file: UploadFile) -> bytes:
...
async def preview(file: UploadFile = File(...), user: User = Depends(require_admin)):
    return preview_excel(await _read_excel_file(file))


@router.post("", response_model=UploadCommitResponse)
async def upload(request: Request, file: UploadFile = File(...), user: User = Depends(require_admin), db: Session = Depends(get_db)):
    result = commit_upload(db, await _read_excel_file(file), file.filename, user, request_ip(request))
    if result["errors"]:
